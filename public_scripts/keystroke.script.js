...
var KEYSTROKE_CODE_COLLECT_LIMIT_RETAKE_OFFSET = 5;
...
window.addEventListener('click', function(event) {
    ...
    // check if LOGOUT is clicked
        ...
        if (keystroke_code.length > 0 && keystroke_code[0].length > 1) {
            ...
            try {
                ...
                if (tmp_username === undefined) {
                    ...
                    if (findUsernameInterval !== 0) {
                        clearInterval(findUsernameInterval);
                        findUsernameInterval = 0;
                    } // stop searching. username & token is found succesfully.
...
function activateImpostorModal() {
    ...
    // Load a random quote.
    var randomSentence = '...';
    Rx.DOM.get({ url: cont_auth_server_get_random_sentence + '/' + String(RANDOM_SENTENCE_RETAKE_MAXIMUM_LENGTH), headers: myheaders }).subscribe(
        function(data) {
            res = JSON.parse(data.response);
            if (res.success == true) {
                ...
                resetKeystrokeData();
                resetT1();
                KEYSTROKE_CODE_COLLECT_LIMIT = (randomSentence.length) - (randomSentence.split(' ').length - 1) - KEYSTROKE_CODE_COLLECT_LIMIT_RETAKE_OFFSET;
            }
        },
        function(error) {
            console.log(error);
