...
for doc in COLLECTION_DOCS:
    ...
    listdict_users_data[le - 1]['di_data'] = \
        [None] * (ALPHABET_LENGTH*ALPHABET_LENGTH) # Empty 27x27 list of vectors
    ...
    for i in range(0, ALPHABET_LENGTH):
        for j in range(0, ALPHABET_LENGTH):
            traindata = listdict_users_data[le-1]['di_data'][i*ALPHABET_LENGTH+j]
            # Ignore empties
            ...
            if len(traindata) < MINIMUM_SAMPLES:
                ...
                myGMM = mixture.GaussianMixture(n_components=N_COMPONENTS).fit(np.reshape(traindata, (-1, 1)))
                # (the gmm model attributes are a little ugly, nested lists etc etc)
                flatMeans = [mean for sublist in myGMM.means_.tolist() for mean in sublist]
                flat2dCovs = [cov for sublist in myGMM.covariances_.tolist() for cov in sublist]
                flat2dCovs = [cov for sublist in flat2dCovs for cov in sublist]
                # also save the digraph itself as string
                str_digraph = (chr(i + DI_CHAR_OFFSET) + chr(j + DI_CHAR_OFFSET)).replace('@','#').lower()
                listdict_users_data[le - 1]['di_gmms'][i*ALPHABET_LENGTH+j] = \
                {'digraph': str_digraph, 'weights':myGMM.weights_.tolist(), 'means':flatMeans, 'covs':flat2dCovs, 'stds': square_list(flat2dCovs)}
