...
module.exports = function(app, express) {
        ...
        .post(function(req, res) {
            ...
            User.findById(req.params.user_id, function(err, user) {
                ...
                if (!user) {
                    ...
                    var proj = { siteurl: req.body.siteurl, track_code: req.body.siteurl.hashCode() };
                    user.projects.push(proj);
                    user.save(function(err) {
                        if (err) return res.send(err);
                        res.json({ success: true, message: 'Project created succesfully', project: user.projects[user.projects.length - 1] });
                    });
    ...
    apiRouter.route('/projects/:user_id')
        .get(function(req, res) {
            ...
            User.findById(req.params.user_id, function(err, user) {
                if (err) return res.send(err);
                if (!user) {
                    res.json({ success: false, message: 'No user with this username exists' });
                } else {
    ...
    apiRouter.route('/project/:user_id/:project_id')
        .get(function(req, res) {
            ...
            User.findById(req.params.user_id, function(err, user) {
                if (err) return res.send(err);
                if (!user) {
                    res.json({ success: false, message: 'Wrong user id' })
                } else {
                    res.json({ success: true, project: user.projects[arrIdFromJsonArray(user.projects).indexOf(req.params.project_id)] })
                }
            });
        })
    ...
    .put(function(req, res) {
        ...
        if (req.body.project == undefined) {
            return res.json({ success: false, message: 'Project missing from body.' })
        }
        ...
        User.findById(req.params.user_id, function(err, user) {
            ...
            if (!user) {
                ...
                if (_projectIndex !== -1) {
                    ...
                    if (bodyProject.enable_keyguard_auth_flag !== undefined) { _project.enable_keyguard_auth_flag = bodyProject.enable_keyguard_auth_flag; }
                    user.projects[_projectIndex] = _project;
                    user.save(function(err1) {
                        if (err1) return res.send(err1);
                        res.json({ success: true, track_code_changed_flag: track_code_changed_flag, message: 'Project changed succesfully', project: user.projects[_projectIndex] });
                    })
                } else {
                    res.json({ success: false, message: 'Project id invalid' })
                }
        ...
        User.findById(req.params.user_id, function(err, user) {
            ...
            if (!user) {
                ...
                user.save(function(err) {
                    ...
                    KeystrokeDataModel.remove({ track_code: track_code_to_delete }, function(err2) {
                        if (err2) return res.send(err2);
                        res.json({ success: true, message: 'Project deleted succesfully', projects: user.projects })
                    });
                });
            }
        });
        ...
        .post(function(req, res) {
            ...
            User.findById(req.params.user_id, function(err1, user) {
                ...
                if (!user) {
                    ...
                    res.json({ success: false, message: 'Your id is invalid' });
                    ...
                    let _projectIndex = arrIdFromJsonArray(user.projects).indexOf(req.params.project_id);
                    if (_projectIndex == -1) {
                        return res.json({ success: false, message: 'Project id invalid' });
                    }
                    ...
                    KeystrokeDataModel.find({ 'track_code': track_code }, function(err, docs) {
                        if (err) console.log('error occured in the database');
                        if (!docs.length) {
                            console.log('Track code doesnt exist in subjects yet.');
                            res.json({ message: 'You do not have any data to train yet!' });
                        } else {
                            // console.log(docs);
                            ...
                            pyshell.on('message', function(message) {
                                ...
                                if (typeof message.data == 'string') {
                                    ...
                                    res.json({ success: true, message: 'Training Completed Succesfully!' })
            ...
            if (!user) {
                ...
                KeystrokeDataModel.find({ track_code: track_code }, function(err, docs) {
                    ...
                    if (!docs.length) {
                        ...
                        return res.json({ success: false, val: 'no-subjects', dashboardData: {} });
                    } else {
                        // console.log(docs);
                        ...
                        docs.forEach(function(doc) {
                            ...
                            // For gen statistics
                            let tempObj = { id: doc._id, is_trained: doc.is_trained, total_keystrokes: 0, min: 12345, max: -1, avg: 0, std: 0, fastest_di: '', slowest_di: '', rejections: doc.rejections };
                            let docDt = [];
                            let docCode = [];
    ...
    .delete(function(req, res) {
        ...
        User.findById(req.params.user_id, function(err, user) {
            ...
            if (!user) {
                ...
                res.json({ success: false, message: 'Problem with your userid.' });
            } else {
                ...
                KeystrokeDataModel.remove({ track_code: track_code }, function(err, obj) {
                    // console.log(obj);
                    if (err) {
                        console.log(err);
                        res.json(err);
                    ...
                    if (obj.result.n == 0) {
                        ...
                        res.json({ success: false, message: 'You do not have any data to delete!' });
                        ...
                        user.save(function(err3) {
                            ...
                            res.json({ success: true, message: 'Data removed succesfully' });
                        });
                    }
                });
            }
    ...
    .get(function(req, res) {
        ...
        User.findById(req.params.user_id, function(err, user) {
            ...
            if (!user) {
                ...
                KeystrokeDataModel.findOne({ _id: req.params.subject_id }, '_id _v subject is_trained track_code sessions di_gmms rejections', function(err, doc) {
                    if (err) {
                        console.log(err)
                        return res.json(err);
                    }
                    if (!doc) {
                        ...
                        res.json({ success: false, message: 'Subject id not found on database' });
                    } else {
    ...
    .delete(function(req, res) {
        ...
        User.findById(req.params.user_id, function(err, user) {
            ...
            if (!user) {
                ...
                // Find the subject with  the req.params.subject_id
                KeystrokeDataModel.remove({ _id: req.params.subject_id }, function(err, obj) {
                    if (err) {
                        console.log(err)
                        return res.json(err);
                    }
                    if (obj.result.n == 0) {
                        ...
                        res.json({ success: false, message: 'Wrong subject id' });
                        ...
                        const track_code = user.projects[_projectIndex].track_code;
                        KeystrokeDataModel.find({ track_code: track_code }, function(err2, docs) {
                            if (err2) return res.send(err2);
                            if (docs.length == 0) {
                                user.projects[_projectIndex].last_train_date = undefined;
                                user.save();
                                res.json({ success: true, train_date_reset: true, message: 'Subject removed succesfully.' })
                            } else {
                                res.json({ success: true, train_date_reset: false, message: 'Subject removed succesfully' })
                            }
                        });
                    }
                });
